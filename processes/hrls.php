    ...
    while ($IDgenerationDone === false) {
        ...
        if ($stmt->execute()) {
            ...
            while ($row = $result->fetch_assoc()) {
                $participantID = str_pad($row['participant_id'], 3, '0', STR_PAD_LEFT);
                $firstName = strtoupper($row['firstname']);
                $lastName = $row['lastname'];
                $text = $row['firstname'] . " " . $row['middle_initial'] . " " . $row['lastname'];
                $agency = $row['agency'];
                ...
                if (file_exists(realpath("$folderName/$IDfilename"))) {
                    ...
                    // Load the image
                    $image = imagecreatefrompng('../sources/rhls-id.png');

                    // ----------------- FOR FULL NAME --------------------------


                    // Define the font and color for the text
                    $font = '../fonts/Poppins/Poppins-Regular.otf'; // Path to your font file
                    $color = imagecolorallocate($image, 0, 0, 0); // Black font color
                    $fontSize = 45; //initial font size
                    ...
                    if ($textWidth > imagesx($image) / 1.7) {
                        while ($textWidth > imagesx($image) / 1.7) {
                            $fontSize = $fontSize - 5;
                    ...
                    $textX = imagesx($image) - 1307; // Center the text horizontally
                    $textY = imagesy($image) - 480; // Adjust the Y position as needed
                    ...
                    // Define the font and color for the text
                    $font1 = '../fonts/Poppins/Poppins-Bold.otf'; // Path to your font file
                    $color = imagecolorallocate($image, 0, 0, 0); // Black font color
                    $fontSize = 95; //initial font size

                    $textBox = imagettfbbox($fontSize, 0, $font1, $firstName); // Do this so you can get the width that the text will take up
                    $textWidth = abs($textBox[4] - $textBox[6]); // Here is the text width
                    $fnHeight = abs($textBox[5] - $textBox[3]); // And this is the height

                    // This if-else statement will reduce the font size of the name if it doesn't fit the image
                    if ($textWidth > imagesx($image) / 1.7) {
                        while ($textWidth > imagesx($image) / 1.8) {
                            $fontSize -= 5;
                    ...
                    $fnameX = imagesx($image) - 1315; // Center the text horizontally
                    $fnameY = $textY - $textHeight - 10; // Adjust the Y position as needed
                    ...
                    $fontID = '../fonts/Poppins/Poppins-Italic.otf';

                    $idx = imagesx($image) - 333;
                    $idy = imagesy($image) - 355;

                    $idBox = imagettfbbox(30, 0, $fontID, $participantID);
                    $idBoxWidth = abs($idBox[2] - $idBox[0]);

                    imagettftext($image, 30, 0, (int) $idx, (int) $idy, $color, $fontID, $participantID);
